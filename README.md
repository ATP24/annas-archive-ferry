...
**从找对版本到拿到可用文件：为 Agent 和命令行设计的文献工作流。**

在 Anna's Archive 查找书目、比较版本、探测文件大小，再由你选择要保存的文件。下载后校验书目 MD5；遇到 DjVu 时，可自动转为更易打开和分享的 PDF。
...
> **项目定位**：这是运行在你电脑上的 Agent Skill 与 Python CLI，使用网站公开页面和下载通道。它不是 Anna's Archive 的官方客户端，也不会提高网站给定的下载速度。请仅获取你有权访问的资料。

## 为什么用它，而不是在网站逐本下载？

网站适合偶尔手动找一本书。本项目适合需要**比较多个版本、让 Agent 执行重复步骤、确认文件确实可用**的场景。

| 常见麻烦 | 本项目提供的帮助 |
| --- | --- |
| 搜索结果多，格式与版本容易选错 | 把书名、格式、来源和书目 MD5 整理成清单，由你选定版本 |
| 点击下载后才发现文件很大或通道不可用 | 先探测直链和服务器报告的大小；失败时明确报错 |
| 传输中断后不确定文件是否完整 | 在服务器支持时续传，并在交付前核对长度和书目 MD5 |
| 得到 DjVu 后还要寻找转换工具 | 检测到 `ddjvu` 时自动转成 PDF，并检查生成的 PDF 是否可打开 |
| Agent 难以读取网页和进度日志 | `search`、`probe` 提供纯 JSON 标准输出，下载显示实测进度 |

它**不会绕过站点限制或保证每次成功**。使用的仍是站点提供的访问和下载通道；价值在于把检索、决策、下载、校验与格式转换接成可重复的本地流程。
...
| 转换 | 下载 DjVu 后自动生成 PDF，原始 DjVu 保留 | `download` + `ddjvu` |
...
需要 **Python 3.9+**。检索与解析链接还需要 Chrome、Edge 或 Playwright Chromium；**DjVu 自动转 PDF** 需要另外安装 `ddjvu`。未安装或转换失败时会保留原始 DjVu。
...
把整个仓库放入你的 Agent 支持的 skills 目录，确保 `SKILL.md` 位于该技能目录顶层，并在同一环境安装上面的 Python 依赖。随后可以说：“在 Anna's Archive 找《傲慢与偏见》的 EPUB，列出版本，先不要下载。” 或“找这本书的 DjVu，选定版本后下载；如果本机有 `ddjvu`，帮我转成 PDF。”
...
- DjVu 转 PDF 由 `ddjvu` 执行，需要单独安装并在系统路径中可找到；转换过程可能改变文件大小和图像表示，不承诺无损或保留可搜索文本层。
